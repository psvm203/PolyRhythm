...
func _init() -> void:
	...
	_expect(ProgressStoreScript.next_unlocked_stage(1, 3) == 3, "replay does not reduce progress")
	_expect(ProgressStoreScript.next_unlocked_stage(3, 3) == 3, "last stage stays capped")
	...
	if _failures == 0:
		print("Progress store tests passed: 8 assertions")
		quit(0)
	else:
